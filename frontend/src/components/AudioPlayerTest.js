...
class AudioPlayerTest extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            index: 3,
            currentTime: '0:00',
            songsList: [{name:'Nice piano and ukulele', author: 'Royalty', img: 'https://www.bensound.com/bensound-img/buddy.jpg', audio:'https://www.bensound.com/bensound-music/bensound-buddy.mp3', duration: '2:02'}, 
    ...
    changeCurrentTime = (event) => {
        ...
        const userClickWidth = event.clientX - offsetWidth;
    ...
    updatePlayer = () => {
        const {songsList, index} = this.state;
    ...
    nextSong = () => {
        const { songsList, index, pause } = this.state;
        this.setState({
    ...
    prevSong = () => {
        const {songsList, index, pause } = this.state;
        this.setState({
            index: (index + songsList.length - 1) % songsList.length
        });
        this.updatePlayer();
        if(pause){
            this.playerRef.play();
    ...
    render() {
        const { songsList, index, currentTime, pause } = this.state;
        const currentSong = songsList[index];
        return (
            <div className="card">
                ...
                <div className="play-list">
                    {songsList.map( (song, key=0) => 
                        <div key={key} onClick={() => this.clickAudio(key)} className={"song " + (index === key && !pause ?'current-audio': '') + (index === key && pause ?'play-now':'')} >
                            <img className="song-img" src={song.img} />
                            <div className="song-discr" >
                                <span className="song-name">{song.name}</span>
                                <span className="song-author">{song.author}</span>
                            </div>
                            <span className="song-duration">
                                {(index === key) ? currentTime : song.duration}
                            </span>
                        </div>
                    )}
                </div>
