...
class SongTestCase(unittest.TestCase):
    ...
    def test_api_can_get_song_by_id(self):
        ...
        result = self.client().get('/songs/{}'.format(result_in_json['id']))
    ...
    def test_song_can_be_edited(self):
        ...
        rv = self.client().post('/songs/', data={'name': 'Dont Know Why'})
        self.assertEqual(rv.status_code, 201)
        rv = self.client().put('/songs/1', data={"name": "Dedicated To You"})
